...
function postCard(p) {
  ...
  return `
    <article class="post-card reveal">
      ${PU.coverImage(p)}
      <div class="post-info">
        <span class="post-tag tag-${p.category}">${p.category}</span>
      ...
      <div class="post-actions">
        ...
        <a class="btn btn-sm btn-outline post-link-xhs" href="${xhs}" data-post-id="${p.id}">见原帖</a>
      </div>
    </article>`;
}
...
function renderTabs(active) {
  ...
  el.innerHTML = Object.entries(REGIONS)
    ...
    .join("");
  document.querySelectorAll(".region-tabs .nav-to-region").forEach((a) => {
    if (window.CrabTransition) {
      a.addEventListener("click", (e) => {
        if (e.metaKey || e.ctrlKey) return;
        e.preventDefault();
        window.CrabTransition.go(a.getAttribute("href"), `去${a.textContent}…`);
      });
    }
  });
}
...
async function init() {
  ...
  try {
    ...
    const posts = all.filter((p) => p.isFood && (p.region || "other") === region);
    const grid = document.getElementById("region-posts");
    if (!posts.length) {
      grid.innerHTML = `<p class="recent-empty">这个地区还没有美食笔记，看看其他地区吧。</p>`;
      return;
    }
